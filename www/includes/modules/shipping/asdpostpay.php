			  ...
			  while (list($city, $state, $street_address, $postcode, $telephone) = fgetcsv($fp, 128, ';')) {
				if (tep_not_null($postcode)) {
				  ...
				  if (preg_match('/^' . $postcode . '.*$/', $order->delivery['postcode'])) {
					...
					if (tep_not_null($telephone)) $telephone = MODULE_SHIPPING_ASDPOSTPAY_TEXT_TELEPHONE . $telephone;
...
// class methods
	function quote($method = '') {
	  ...
	  list($asd_standart_kg, $asd_standart_next_kg) = explode(':' , MODULE_SHIPPING_ASDPOSTPAY_STANDART_COAST);
	  list($asd_pickup_kg, $asd_pickup_next_kg) = explode(':' , MODULE_SHIPPING_ASDPOSTPAY_PICKUP_COAST);

	  $methods = array();

	  if ($method!='pickup') {
		$detail['id'] = 'courier';
		//АСД-Мегаполис-Стандарт (курьерская) с оплатой при получении
		$detail['title'] = (tep_not_null($method) ? '' : MODULE_SHIPPING_ASDPOSTPAY_TEXT_SIPPING_TYPE_EXPRESS);
		$detail['cost'] = $this->coast($total['sum'], $total['weight'], $asd_standart_kg, $asd_standart_next_kg, MODULE_SHIPPING_ASDPOSTPAY_PREMIUM, MODULE_SHIPPING_ASDPOSTPAY_COD);
		$methods[] = $detail;
	  }

	  if (tep_not_null($this->pickup) && $method!='courier') {
		//АСД-Мегаполис-Эконом (самовывоз) с оплатой при получении
		$detail['id'] = 'pickup';
		$detail['title'] = (tep_not_null($method) ? trim($this->pickup) : MODULE_SHIPPING_ASDPOSTPAY_TEXT_SIPPING_TYPE_PICKUP . ': ' . trim($this->pickup));
		$detail['cost'] = $this->coast($total['sum'], $total['weight'], $asd_pickup_kg, $asd_pickup_next_kg, MODULE_SHIPPING_ASDPOSTPAY_PREMIUM, MODULE_SHIPPING_ASDPOSTPAY_COD);
		$methods[] = $detail;
	  }

	  $this->quotes = array('id' => $this->code,
							'module' => (tep_not_null($method) ? ($method=='pickup' ? MODULE_SHIPPING_ASDPOSTPAY_TEXT_TITLE_PICKUP : MODULE_SHIPPING_ASDPOSTPAY_TEXT_TITLE_COURIER) : MODULE_SHIPPING_ASDPOSTPAY_TEXT_TITLE),
							'methods' => $methods);
	...
	function check() {
	  if (!isset($this->_check)) {
		$check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_ASDPOSTPAY_STATUS'");
	...
	function params() {
	  ...
	  $install_params[] = array(
			...
			'configuration_key' => 'MODULE_SHIPPING_ASDPOSTPAY_STATUS',
	  ...
	  $install_params[] = array(
			...
			'configuration_key' => 'MODULE_SHIPPING_ASDPOSTPAY_STANDART_COAST',
	  ...
	  $install_params[] = array(
			...
			'configuration_key' => 'MODULE_SHIPPING_ASDPOSTPAY_PICKUP_COAST',
	  ...
	  $install_params[] = array(
			...
			'configuration_key' => 'MODULE_SHIPPING_ASDPOSTPAY_TABLE_FILE_NAME',
	  ...
	  $install_params[] = array(
			...
			'configuration_key' => 'MODULE_SHIPPING_ASDPOSTPAY_COURIER_ZONE',
			'configuration_value' => '',
	  ...
	  $install_params[] = array(
			...
			'configuration_key' => 'MODULE_SHIPPING_ASDPOSTPAY_PICKUP_ZONE',
			'configuration_value' => '',
	  ...
	  $install_params[] = array(
			...
			'configuration_key' => 'MODULE_SHIPPING_ASDPOSTPAY_PREMIUM',
	  ...
	  $install_params[] = array(
			'configuration_title' => 'Наложенный платеж',
			'configuration_key' => 'MODULE_SHIPPING_ASDPOSTPAY_COD',
			'configuration_value' => '5',
			'configuration_description' => 'Укажите (в процентах) стоимость наложенного платежа для этого вида доставки',
			'configuration_group_id' => '6',
			'sort_order' => '60',
			'date_added' => 'now()',
			);
	  $install_params[] = array(
			...
			'configuration_key' => 'MODULE_SHIPPING_ASDPOSTPAY_SORT_ORDER',
