 ...
 */
    ...
    function send($to_name, $to_addr, $from_name, $from_addr, $subject = '', $headers = '') {
      $to = (($to_name != '') ? '"' . $to_name . '" <' . strtolower($to_addr) . '>' : strtolower($to_addr));
      $from = (($from_name != '') ? '"' . $from_name . '" <' . strtolower($from_addr) . '>' : strtolower($from_addr));
	  ...
	  if ($fd = @popen('/usr/sbin/sendmail -t -f ' . $from_addr, 'w')) {
		fputs($fd, $additional_headers . "\n\n");
		fputs($fd, $this->output . "\n");
		pclose($fd);
		$mailed = true;
	  }
