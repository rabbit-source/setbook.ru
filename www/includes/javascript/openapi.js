...
VK.Auth = {
  ...
  logout: function(cb) {
    ...
    VK.Api.attachScript('http://vkontakte.ru/logout.php?openapi=1&app='+parseInt(VK._apiId)+'&mid='+VK._session.mid+'&sid='+VK._session.sid+'&rnd='+parseInt(Math.random()*10000));
...
if (!VK.Widgets) {
  ...
  VK.Widgets.Like = function(objId, options, page) {
    ...
    var 
        ...
        initTooltip: function (counter) {
          tooltipRpc = new fastXDM.Server({
            ...
            statsBox: function (act) {
              ...
              scrollTop = options.getScrollTop ? options.getScrollTop() : Math.max(parseInt(window.pageYOffset) || 0, document.documentElement.scrollTop,  document.body.scrollTop),
              top = Math.max(0, scrollTop + (height - sizes[1]) / 3);

              if (!statsIfr) {
                statsRpc = new fastXDM.Server({
                  onInit: function () {
                    statsIfr.style.background = 'transparent';
                  },
                  hide: function () {
                    statsIfr.style.display = 'none';
                  }
                });
                statsIfr = statsRpc.append(document.body, {
                  src: buttonIfr.src + '&act=a_stats_box',
                  scrolling: 'no',
                  allowTransparency: true,
                  id: buttonIfr.id + '_box',
                  style: {position: 'absolute', left: '50%', zIndex: 1002, background: 'http://vkontakte.ru/images/upload.gif center center no-repeat', padding: '0', border: '0', width: sizes[0] + 'px', height: sizes[1] + 'px', overflow: 'hidden'}
                });
              }
              ss(statsIfr, {'top': top + 'px', margin: '0 0 0 ' + (-sizes[0]/2) + 'px', display: 'block'});
            }
          });
          tooltipIfr = tooltipRpc.append(ttHere ? obj : document.body, {
            ...
            allowTransparency: true,
            id: buttonIfr.id + '_tt',
            style: {position: 'absolute', padding: 0, display: 'block', visibility: 'hidden', border: '0', width: '206px', height: '127px', zIndex: 140, overflow: 'hidden'}
          });
      ...
      tooltipIfr, tooltipRpc, isOver = false, checkTO, statsIfr, statsRpc,
      obj = res[0],
      buttonIfr = res[1],
      buttonRpc = res[2];
    ss(obj, {height: height + 'px', width: width + 'px', position: 'relative', clear: 'both'});
    ...
    function showTooltip(force) {
      ...
      ss(tooltipIfr, {left: ((ttHere ? 0 : objPos[0]) - (type == 'vertical' || type == 'mini' ? 36 : 2)) + 'px', display: 'block', visibility: 'visible'});
      isOver = true;
    };
  ...
  VK.Widgets.Auth = function(objId, options) {
    ...
    VK.Widgets._constructor('widget_auth.php', objId, options, {}, {makeAuth: function(data) {
      ...
      if (options.onAuth) {
        ...
        for (var i in data) {
          if (i != 'session') vars.push(i+'='+data[i]);
        }
  ...
  VK.Widgets._constructor = function(widgetUrl, objId, options, params, funcs, defaults) {
    ...
    VK.Widgets.RPC[widgetId] = new fastXDM.Server(funcs);
