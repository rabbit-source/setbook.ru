...
// TABLES: information_to_sections
  ...
  function tep_email_log($log)
  {
    $email_log_file = @fopen(SESSION_WRITE_DIRECTORY . '/mail_log.txt', 'a');
    if ($email_log_file)
    {
      fwrite($email_log_file, '[' . date('h:i:s') . '] ' . $log . PHP_EOL);
      fclose($email_log_file);
    }
  }
