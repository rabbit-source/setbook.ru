    ...
    switch ($action) {
      ...
      case 'insert':
        if (isset($HTTP_GET_VARS['cID'])) {
		  ...
		  $max_id_query = tep_db_query("select max(countries_id) as new_id from " . TABLE_COUNTRIES . "");
		  $max_id = tep_db_fetch_array($max_id_query);
		  $countries_id = (int)$max_id['new_id'] + 1;
		}
		...
		for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
		  $countries_name_array = $HTTP_POST_VARS['countries_name'];

		  $language_id = $languages[$i]['id'];

		  $sql_data_array = array('countries_name' => tep_db_prepare_input($countries_name_array[$language_id]),
		  						  'countries_iso_code_2' => tep_db_input($countries_iso_code_2),
		  						  'countries_iso_code_3' => tep_db_input($countries_iso_code_3),
		  ...
		  if ($action == 'insert') {
			...
			tep_db_perform(TABLE_COUNTRIES, $sql_data_array, 'update', "countries_id = '" . (int)$countries_id . "' and language_id = '" . (int)$language_id . "'");
		  }
		}
...
<?php
  $countries_query_raw = "select * from " . TABLE_COUNTRIES . " where language_id = '" . (int)$languages_id . "' order by sort_order, countries_name";
...
?>
              <tr>
                <td colspan="4"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $countries_split->display_count($countries_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $HTTP_GET_VARS['page'], TEXT_DISPLAY_NUMBER_OF_RECORDS); ?></td>
...
<?php
  ...
  switch ($action) {
    case 'new':
	  ...
	  $languages = tep_get_languages();
	  for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
		$countries_inputs_string .= '<br>' . tep_image(DIR_WS_CATALOG_IMAGES . $languages[$i]['image'], $languages[$i]['name'], '', '', 'style="float: left; margin: 4px 4px 0px 0px;"') . tep_draw_input_field('countries_name[' . $languages[$i]['id'] . ']', '', 'size=32');
	  }
