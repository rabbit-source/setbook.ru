  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'insert':
        ...
        break;
      case 'save':
        $zone_id = tep_db_prepare_input($HTTP_GET_VARS['zID']);
        $zone_country_id = tep_db_prepare_input($HTTP_POST_VARS['zone_country_id']);
        $zone_code = tep_db_prepare_input($HTTP_POST_VARS['zone_code']);
        $zone_name = tep_db_prepare_input($HTTP_POST_VARS['zone_name']);
        ...
        tep_redirect(tep_href_link(FILENAME_ZONES, 'page=' . $HTTP_GET_VARS['page'] . '&zID=' . $zone_id));
        break;
      case 'deleteconfirm':
        $zone_id = tep_db_prepare_input($HTTP_GET_VARS['zID']);
...
<?php
  if (isset($HTTP_GET_VARS['zPath'])) {
	...
	$shippings_modules = array();
	$installed_modules = array();
	$installed_modules[] = array('id' => '', 'text' => TEXT_CHOOSE);
	$modules = array();
	for ($i=0, $j=1, $n=sizeof($directory_array); $i<$n; $i++) {
	  ...
	  if (tep_class_exists($class)) {
		$module = new $class;
		if ($module->check() > 0) {
		  $installed_modules[] = array('id' => $class, 'text' => $module->title);
		  $shippings_modules[$class] = $module->title;
		  $modules[$j] = $module->title;
		  $j ++;
		}
	  }
	}
?>
			<table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                <td class="dataTableHeadingContent"><?php echo TABLE_HEADING_CITY_NAME; ?></td>
                <td class="dataTableHeadingContent"><?php echo TABLE_HEADING_CITY_CODE; ?></td>
                <td class="dataTableHeadingContent"><?php echo TABLE_HEADING_SUBURB_NAME; ?></td>
              </tr>
<?php
	$rows = 0;
	$cities_query = tep_db_query("select z.zone_id, z.zone_name, c.city_id, c.city_name, c.suburb_name from " . TABLE_ZONES . " z, " . TABLE_CITIES . " c where z.zone_id = c.zone_id and " . (strlen($HTTP_GET_VARS['zPath'])==6 ? "c.parent_id = '" . (int)$HTTP_GET_VARS['zPath'] . "'" : "z.zone_id = '" . (int)$HTTP_GET_VARS['zPath'] . "' and c.parent_id = '0'") . " group by c.city_name order by c.city_name, z.sort_order, z.zone_name");
	while ($cities = tep_db_fetch_array($cities_query)) {
	  echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)">' . "\n";
?>
                <td class="dataTableContent"><?php echo (tep_has_city_subcities($cities['city_id'], $cities['city_name']) ? '<a href="' . tep_href_link(FILENAME_ZONES, 'zPath=' . $cities['city_id']) . '">' . tep_image(DIR_WS_ICONS . 'folder.gif', ICON_FOLDER, '16', '16', 'style="margin: 2px 0 -2px 0;"') . '</a>&nbsp;' : '') . $cities['city_name']; ?></td>
				<td class="dataTableContent"><?php echo $cities['city_id']; ?></td>
				<td class="dataTableContent"><?php echo $cities['suburb_name']; ?></td>
              </tr>
<?php
	  $rows ++;
	}
...
<?php
	$zones_query_raw = "select z.zone_id, c.countries_id, c.countries_name, z.zone_name, z.zone_code, z.zone_country_id from " . TABLE_ZONES . " z, " . TABLE_COUNTRIES . " c where z.zone_country_id = c.countries_id order by c.countries_name, z.sort_order, z.zone_name";
	...
	while ($zones = tep_db_fetch_array($zones_query)) {
	  if ((!isset($HTTP_GET_VARS['zID']) || (isset($HTTP_GET_VARS['zID']) && ($HTTP_GET_VARS['zID'] == $zones['zone_id']))) && !isset($zInfo) && (substr($action, 0, 3) != 'new')) {
		$zInfo = new objectInfo($zones);
	  }

	  if (isset($zInfo) && is_object($zInfo) && ($zones['zone_id'] == $zInfo->zone_id)) {
		echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_ZONES, 'page=' . $HTTP_GET_VARS['page'] . '&zID=' . $zInfo->zone_id . '&action=edit') . '\'">' . "\n";
	  } else {
		echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_ZONES, 'page=' . $HTTP_GET_VARS['page'] . '&zID=' . $zones['zone_id']) . '\'">' . "\n";
	  }
?>
                <td class="dataTableContent"><?php echo $zones['countries_name']; ?></td>
                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_ZONES, 'zPath=' . $zones['zone_id']) . '">' . tep_image(DIR_WS_ICONS . 'folder.gif', ICON_FOLDER, '16', '16', 'style="margin: 2px 0 -2px 0;"') . '</a>&nbsp;' . $zones['zone_name']; ?></td>
                <td class="dataTableContent" align="center"><?php echo $zones['zone_code']; ?></td>
                <td class="dataTableContent" align="right"><?php if (isset($zInfo) && is_object($zInfo) && ($zones['zone_id'] == $zInfo->zone_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link(FILENAME_ZONES, 'page=' . $HTTP_GET_VARS['page'] . '&zID=' . $zones['zone_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
	}
?>
              <tr>
                <td colspan="4"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $zones_split->display_count($zones_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $HTTP_GET_VARS['page'], TEXT_DISPLAY_NUMBER_OF_RECORDS); ?></td>
                    <td class="smallText" align="right"><?php echo $zones_split->display_links($zones_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $HTTP_GET_VARS['page'], tep_get_all_get_params(array('page'))); ?></td>
                  </tr>
<?php
