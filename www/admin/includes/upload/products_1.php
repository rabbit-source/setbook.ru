...
// IDKnigi	Name	ISBN	DopISBN	KolPages	Year	Pereplet	Format	Annot	Ves	Tiraj	IDSeria	IDAvtor	IDIzdat	KodTematika	New	DopTiraj	Lider	Rekomenduem	Anons	Elements	Foto	Rubriki
	...
	if ((int)$original_products_code > 0) {
	  ...
	  $products_check_query = tep_db_query("select products_id, products_model, series_id, authors_id, manufacturers_id, products_image_exists, products_image, products_filename, products_md5_sum from " . TABLE_TEMP_PRODUCTS . " where products_code = '" . tep_db_input($products_code) . "' and products_types_id = '" . (int)$products_types_id . "'");
	  ...
	  if ( ($new_status + $reprint_status + $bestseller_status + $recommended_status + $soon_status) > 0) $products_check['products_md5_sum'] = '';
...
//		}
	  ...
	  if ($products_id > 0 && $additional_images_count==0 && sizeof($products_filenames)==0 && sizeof($products_images)==0 && empty($products_check['products_filename']) && $products_check['products_md5_sum']==$products_md5_sum) {
	  ...
	  if ($products_types_id==1) {
		...
		$products_description = tep_db_prepare_input(strip_tags($products_description));
...
//		  $used_authors[$author_code] = array('id' => $authors_id, 'name' => $authors_name);
	  ...
	  while (list(, $category_code) = each($categories)) {
		if (!in_array($category_code, array_keys($used_categories))) {
		  ...
		  $used_categories[$category_code] = array('id' => (int)$category_info['categories_id']);
		  $categories_ids[] = $category_info['categories_id'];
		} else {
		  $categories_ids[] = $used_categories[$category_code]['id'];
		}
	  }
...
tep_mail('sivkov@setbook.ru', 'sivkov@setbook.ru', 'Шаг 5-2', '', STORE_NAME, STORE_OWNER_EMAIL_ADDRESS);
	...
	$max_specials_date_query = tep_db_query("select max(specials_date_added) as specials_date_added from " . TABLE_TEMP_SPECIALS . " where status = '1'");
	...
	$query = tep_db_query("update " . TABLE_TEMP_PRODUCTS . " p left join " . TABLE_TEMP_SPECIALS . " s on (s.products_id = p.products_id and s.specials_types_id in ('1', '2') and s.specials_date_added >= '" . tep_db_input($min_specials_date_added) . "') set p.sort_order = (if(p.products_listing_status=1, 8, 0) + if(s.specials_types_id, if(s.specials_types_id=1, 4, if(s.specials_types_id=2, 3, 0)), 0) + if(p.products_image_exists=1, 2, 0))");
