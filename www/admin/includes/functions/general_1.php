...
#1 = GIF, 2 = JPG, 3 = PNG, 4 = SWF, 5 = PSD, 6 = BMP, 7 = TIFF(intel byte order), 8
  ...
  function tep_update_shops_prices($shop_id = '', $currency_code = '', $table = 'real') {
	global $currencies, $temp_currencies, $languages_id;
	...
	$shops_query = tep_db_query("select shops_id, shops_database, shops_price_equation, shops_shipping_days, shops_currency, shops_default_status from " . TABLE_SHOPS . " where shops_database <> ''" . ($shop_id>0 ? " and shops_id = '" . (int)$shop_id . "'" : "") . (tep_not_null($currency_code) ? " and shops_currency like '" . tep_db_input($currency_code) . "%'" : ""));
	while ($shops = tep_db_fetch_array($shops_query)) {
...
/*
	if (tep_db_table_exists(DB_DATABASE, TABLE_TEMP_PRODUCTS_INFO)) {
	  $empty_categories_products_query = tep_db_query("select products_id from " . TABLE_TEMP_PRODUCTS_INFO . " where products_types_id = '" . (int)$products_types_id . "' and (categories_name = '' or categories_name is null or categories_id = '0' or categories_id is null)");
	  while ($empty_categories_products = tep_db_fetch_array($empty_categories_products_query)) {
		$categories_products_query = tep_db_query("select categories_id from " . TABLE_PRODUCTS_TO_CATEGORIES . " where products_id = '" . (int)$empty_categories_products['products_id'] . "' order by categories_id desc limit 1");
		$categories_products = tep_db_fetch_array($categories_products_query);
		if ($categories_products['categories_id'] > 0) {
		  $categories_name = tep_get_category_name($categories_products['categories_id'], $languages_id);
		  tep_db_query("update " . $shop_db . "." . TABLE_TEMP_PRODUCTS_INFO . " set categories_id = '" . (int)$categories_id . "', categories_name = '" . tep_db_input($categories_name) . "' where products_id = '" . (int)$empty_categories_products['products_id'] . "'");
		} else {
//		  tep_db_query("insert into " . TABLE_PRODUCTS_TO_CATEGORIES . " (products_id, categories_id) values ('" . (int)$empty_categories_products['products_id'] . "', '0')");
		}
	  }
	}
*/
	...
	while ($shops = tep_db_fetch_array($shops_query)) {
	  ...
	  while (list($step, $temp_table) = each($temp_tables)) {
		if ($shops['shops_default_status']=='0') {
		  ...
		  if (tep_db_table_exists(DB_DATABASE, 'temp_' . $temp_table)) {
//			tep_db_query("replace into " . $shop_db . ".temp_" . $temp_table . " select * from " . DB_DATABASE  . ".temp_" . $temp_table);
		  }
		}
	  }

	  tep_db_query("update " . $shop_db . "." . TABLE_TEMP_PRODUCTS . ", tp " . $shop_db . "." . TABLE_PRODUCTS . " p set tp.products_status = p.products_status, tp.products_listing_status = p.products_listing_status, tp.products_xml_status = p.products_xml_status, tp.products_cost = p.products_cost, tp.products_another_cost = p.products_another_cost, tp.products_purchase_cost = p.products_purchase_cost, tp.products_price = p.products_price, tp.products_available_in = p.products_available_in, tp.products_last_modified = now() where tp.products_id = p.products_id");
...
*/
	  ...
	  list($shop_currency) = array_map('trim', explode(',', $shops['shops_currency']));
	  if (in_array($shop_currency, array_keys($temp_currencies))) {
		tep_db_query("update " . TABLE_CURRENCIES . " set value = '" . tep_db_input($temp_currencies[$shop_currency]) . "', last_updated = now() where code = '" . tep_db_input($shop_currency) . "'");
	  }
	...
	reset($temp_currencies);
	while (list($currency_code, $currency_value) = each($temp_currencies)) {
	  tep_db_query("update " . TABLE_CURRENCIES . " set value = '" . tep_db_input($currency_value) . "', last_updated = now() where code = '" . tep_db_input($currency_code) . "'");
	}
...
//	if (tep_db_num_rows($order_info_query) < 1) return false;
	...
	$shop_info_query = tep_db_query("select shops_url, shops_ssl, shops_prefix from " . TABLE_SHOPS . " where shops_id = '" . (int)$order->info['shops_id'] . "'");
	...
	if (!file_exists($order_file)) {
	  ...
	  while ((list($country_name, $country_ru_name, $country_iso_code_2, $country_iso_code_3) = fgetcsv($fc, 40000, ";")) !== FALSE) {
		...
		if ($order_delivery_country==$country_name || $order_delivery_country==$country_ru_name) {
	  ...
	  if ($order_delivery_country_code=='') {
		...
		while ((list($country_name, $country_ru_name, $country_iso_code_2, $country_iso_code_3) = fgetcsv($fc, 40000, ";")) !== FALSE) {
		  $country_name = strtolower($country_name);
		  $country_ru_name = strtolower($country_ru_name);
		  if (strpos($country_name, $order_delivery_country)!==false || strpos($country_ru_name, $order_delivery_country)!==false) {
			$order_delivery_country_code = $country_iso_code_2;
			break;
		  }
		}
