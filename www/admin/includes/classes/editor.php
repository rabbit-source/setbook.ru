...
// Render the editor IFRAME.
	...
	function IsCompatible() {
	  ...
	  if (isset($HTTP_USER_AGENT)) $sAgent = $HTTP_USER_AGENT;
	  else $sAgent = $_SERVER['HTTP_USER_AGENT'];
	...
	function GetConfigFieldString() {
	  ...
	  while (list($sKey, $sValue) = each($this->Config)) {
#	  foreach ($this->Config as $sKey => $sValue) {
		if ($bFirst == false) $sParams .= '&amp;';
		else $bFirst = false;

		if ($sValue === true) $sParams .= $this->EncodeConfig($sKey) . '=true';
		else if ($sValue === false) $sParams .= $this->EncodeConfig($sKey) . '=false';
