<?php
  ...
  $set = (isset($HTTP_GET_VARS['set']) ? $HTTP_GET_VARS['set'] : 'payment');
  ...
  switch ($action) {
    case 'edit':
      ...
      while (list($key, $value) = each($mInfo->keys)) {
        $keys .= ((tep_not_null($keys)) ? '<br><br>' : '') . '<strong>' . $value['title'] . '</strong><br>' . $value['description'] . '<br>';
    ...
    default:
      ...
      if ($mInfo->status == '1') {
        ...
        while (list(, $value) = each($mInfo->keys)) {
          $keys .= ((tep_not_null($keys)) ? '<br><br>' : '') . '<strong>' . $value['title'] . '</strong><br>';
        ...
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=remove') . '">' . tep_image_button('button_module_remove.gif', IMAGE_MODULE_REMOVE) . '</a> <a href="' . tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a>');
